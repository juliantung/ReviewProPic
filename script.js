    ...
    if (reviewPhotos.length > 0) {
        ...
        Array.from(reviewPhotos).forEach((photo, index) => {
            ...
            reader.onload = function(e) {
                compressImage(e.target.result, 300, 300, (compressedUrl) => {
                    photoUrls[index] = compressedUrl;
                    loadedImagesCount++;
...
function saveData() {
    ...
    const compressedDataStr = LZString.compressToEncodedURIComponent(dataStr);
    const dataOutput = document.getElementById('data-output');
    dataOutput.value = compressedDataStr;
    copyToClipboard(compressedDataStr);
}

function loadData() {
    const dataInput = document.getElementById('data-input').value;
    const decompressedDataStr = LZString.decompressFromEncodedURIComponent(dataInput);
    const undoneContainer = document.getElementById('undone-container');
